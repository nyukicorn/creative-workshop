...
const server = new Server({ name: "threejs_mcp_server", version: "1.0.0" }, { capabilities: { prompts: {}, tools: {} } });
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (name === "addObject") {
        ...
        const command = { action: "addObject", ...input };
        ...
        const command = { action: "moveObject", ...input };
        clientConnection.send(JSON.stringify(command));
        return {
            content: [
                {
        ...
        const command = { action: "removeObject", ...input };
        clientConnection.send(JSON.stringify(command));
        return {
            content: [
                {
        ...
        try {
            const blenderAssetPath = input?.blenderAssetPath;
            const targetGLB = input?.targetGLB;
...
function createViewerHTML(glbPath, title) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
